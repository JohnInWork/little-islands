...
/**
 * Вещь не подбирается сама.
 *
 * Иван: «может быть такое, что игроку что-то выпало, и он сразу это поднял, и
 * он даже не успел понять, что случилось». Раньше добыча уезжала в рюкзак от
 * того, что герой прошёл по клетке, — и увидеть её можно было только потом, в
 * списке. Теперь это отдельное нажатие на кнопку с картинкой самой вещи.
 *
 * Монеты — исключение, и нарочное: горсть золота это число, рассматривать в
 * ней нечего, а кнопка на каждой превратила бы дорогу в перечисление.
 */
test('вещь с пола берут руками, а монеты подбираются сами', async () => {
  ...
  /*
   * И самое дорогое место: расстояние до цели.
   *
   * Вещь на полу хранит своё место в пикселях, как всё живое, а не в клетках,
   * как двери и костры. Забыть её в этом списке — значит получить кнопку,
   * которая появляется и не нажимается; так и вышло с первого раза.
   */
  assert.match(
    functionBody(runtime, 'contextTargetIsAdjacent'),
    /entry\.kind === 'loot'/,
