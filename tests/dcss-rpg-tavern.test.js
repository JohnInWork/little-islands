...
/**
 * Пол и стены дома должны различаться на глаз.
 *
 * Досок в наборе один комплект, и он лёг и на стены, и на полы: внутри дома
 * одна текстура сходилась сама с собой, и комната читалась как бурое пятно.
 * Иван: «очень сильно сливается деревянный пол с деревянными стенами».
 * Различает их свет, и разница проверяется числом — иначе следующая правка
 * тинта вернёт пятно молча.
 */
test('в городе стены заметно темнее пола, иначе дом — бурое пятно', async () => {
  const { biomeThemeFor } = await import('../tools/dcss-rpg-visuals.js');
  ...
  const пол = яркость(town.world3d.floorTint);
  const стены = яркость(town.world3d.wallTint);
  assert.ok(пол - стены > 0.2, `пол ${пол.toFixed(2)} и стены ${стены.toFixed(2)} снова сливаются`);
  // Но не до черноты: город остаётся тёплым деревянным, а не угольным.
  assert.ok(стены > 0.4, 'стены притушены до угля');
});

/**
 * Обед не стоит как броня.
 *
 * Цены трактира стояли руками и разошлись с рынком втрое: четырнадцать за
 * хлеб, который в лавке пять, и шестьдесят за тарелку — почти цена лучшего
 * топора в игре. Иван: «еда в таверне какая-то безумно дорогая, а, например,
 * какая-нибудь железная кираса очень дешёвая. <...> еду я как бы буду покупать
 * чаще, понимаешь».
 *
 * Здесь заперты обе половины отношения: трактир дороже рынка, потому что
 * готовит и подаёт, — и дешевле хорошей брони, потому что это обед.
 */
test('трактир дороже рынка, но дешевле хорошей брони', async () => {
  const { LOOT_CATALOG, lootById } = await import('../tools/dcss-rpg-content.js');
  const { merchantBuyPrice } = await import('../tools/dcss-rpg-merchant.js');
  const { TAVERN_MENU } = await import('../tools/dcss-rpg-tavern.js');
  for (const { itemId, price } of TAVERN_MENU) {
    const рынок = merchantBuyPrice(lootById(itemId));
    assert.ok(price > рынок, `${itemId}: в трактире не дороже рынка`);
    assert.ok(price <= рынок * 2, `${itemId}: ${price} против рыночных ${рынок} — это уже не наценка`);
  }
  const самыйДорогойОбед = Math.max(...TAVERN_MENU.map(({ price }) => price));
  const дешёвоеРедкое = Math.min(...LOOT_CATALOG
    .filter((item) => item.slot && item.randomDrop !== false && (item.rarity ?? 0) >= 2)
    .map((item) => merchantBuyPrice(item)));
  assert.ok(
    самыйДорогойОбед < дешёвоеРедкое,
    `обед за ${самыйДорогойОбед} против редкой вещи за ${дешёвоеРедкое}`,
  );
});
