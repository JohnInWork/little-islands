...
export const BIOME_THEMES = Object.freeze([
  ...
  Object.freeze({
    ...
    world3d: Object.freeze({
      /*
       * Пол светлый, стены тёмные — иначе дом внутри одно бурое пятно.
       *
       * Тёплый кремовый тинт пола желтил траву до соломы: под ним зелёный
       * переставал быть зелёным, поэтому пол берёт почти чистый свет.
       *
       * А стены пришлось притушить: доски в наборе одни и те же и на стену, и
       * на пол, так что внутри дома одна и та же текстура сходилась сама с
       * собой. Иван: «очень сильно сливается деревянный пол с деревянными
       * стенами». Разводит их не рисунок, а свет — тёмное дерево стен против
       * светлого пола; снаружи город при этом остаётся тёплым, а не серым.
       */
      floorTint: '#f6fbf2',
      wallTint: '#ab7a4f',
      actorTint: '#f4ece0',
