...
function triggerFloorEvent(event) {
  ...
  if (effect === 'heal') {
    ...
    hero.power += value;
    playSound('spell-toggle');
    showLootToast({ path, rarity: 3 }, formatDeltas({ attack: value }));
...
function interactNearbyFind(preferredFind = null, action = null, { magicKey = false } = {}) {
  ...
  if (result.rewardPower > 0) {
    addCombatGlyph(hero.x, hero.y, `+${result.rewardPower}{attack}`, presentation.color, -62);
  }
  /*
   * Иван: кристалл дал ему «+5», и он не понял, чего. Строка была
   * «+1 · 5●»: единица силы удара без слова и золото типографским кружком.
   * Теперь это «+1 [меч] к силе удара · +5 [монета] золота».
   */
  ...
  showLootToast(
    ...
    awakened.length > 0
      ...
      : findDeltas || feedbackCopy(itemDetailLanguage).nothingFound,
    awakened.length > 0 ? 'hostile' : null,
  );
  if (result.noise > 0) alertNearbyMonsters(find.x, find.y, result.noise);
...
function useConsumable(item, index, effectOverride = null) {
  ...
  if (item.identification?.group === 'potion') {
    ...
    feedback = consumableReport().power(effect.amount);
    feedbackKind = 'buff';
  } else {
